...
if ~isempty(functionKeep)
    ...
    BiomassRelatedRxns = fastcore(mediumConstrainedmodel, B, epsilon, 0, C);
   BiomassRelatedRxnsID= find(ismember(consistentModel.rxns,BiomassRelatedRxns.rxns));
    C = union(C,BiomassRelatedRxnsID); % add the ATP and biomass reactions to the core set
...
notexpressed = setdiff(notexpressed,BiomassRelatedRxnsID);
...
model = fastcore(ConsistentmediumConstrainedmodel, C, 1e-4, 0, NonPenalised);
else
    disp( 'Statistics and Machine Learning and Curve fitting toolbox must be installed')                         
end
%% building of the context-specific model
Afinal = find(ismember(modelOrg.rxns,model.rxns));
end
